...
opts = OptionParser.new do |opts|
  ...
  opts.on( '-x', '--string-threshold value', Float, 'String distance threshold between [0,1] - Default=0.7' ) do |c|
    options[:stringthreshold] = c
  end
   options[:rdsthreshold] = nil
  opts.on( '-y', '--rds-threshold value', Float, 'RDS threshold between [0,1] - Default=max(media,mean)' ) do |c|
