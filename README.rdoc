...
Note that virtuoso does not have one repository for each dataset. It has just one repository and it organizes the datasets in NAMED GRAPH. 
...
//For RDF format
DB.DBA.RDF_LOAD_RDFXML_MT (file_to_string_output ('/home/user/samur/data/all-geonames.rdf'), '', 'http://geonames.org');

//For NT format
DB.DBA.TTLP_MT (file_to_string_output ('/home/user/samur/data/all-geonames.ttl'), '','http://geonames.org');
...
We have to install the following gems in order to make Serimi works properly:
