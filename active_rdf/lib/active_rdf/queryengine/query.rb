...
class Query
  attr_reader :adapter_to_use,:insert_clauses,:delete_clauses, :select_clauses, :where_clauses, :sort_clauses, :keywords, :limits, :optional_clauses, :offsets, :reverse_sort_clauses, :filter_clauses, :sparql_query
  
  bool_accessor :distinct, :ask, :select, :count, :keyword, :reasoning, :insert, :delete
  ...
  def insert s,p,o,c=nil      
    @insert=true 
    @insert_clauses = [s,p,o,c]     
#    puts @insert_clauses
  ...
  def sort *s
    ...
    s.each { |clause| @sort_clauses << (clause) }
    @sort_clauses.uniq!    
    self
  end  
  ...
  # variable is Ruby symbol that appears in select/where clause, regex is sparql 
  # regular expression
  def filter_regexp(variable, regexp)
    ...
    filter "regex(str(?#{variable}),'#{regexp}','i')"
  end
  ...
  def reverse_sort *s
    # add sort clauses without duplicates
    s.each { |clause| @reverse_sort_clauses << (clause) }
  ...
  def optional s,p,o,c=nil
    ...
    unless s.respond_to?(:uri) or s.is_a?(Symbol)
      raise(ActiveRdfError, "cannot add a option clause with s #{s}: s must be a resource or a variable")
    end
    unless p.respond_to?(:uri) or p.is_a?(Symbol)
      raise(ActiveRdfError, "cannot add a option clause with p #{p}: p must be a resource or a variable")
    end
    
    @optional_clauses << [s,p,o,c]
